...
export class Client {
    ...
    private readonly clean_timeout: number

    constructor({host, port, dbNumber, password, cleanTimeout}: {
        host: string,
        port: number,
        dbNumber: number,
        password: string,
        ...
        this.clean_timeout = cleanTimeout
        setInterval(() => this.clearExpiredSessions(this.clean_timeout), this.clean_timeout)
    }
